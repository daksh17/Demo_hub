...
| **[`k8s/`](k8s/README.md)** | **Kubernetes** — same demo-hub stack as **`docker-compose.yml`**, generated by **`k8s/scripts/gen_demo_hub_k8s.py`** (**Secrets**, Cassandra **PVCs**, **Ingress / PDB / HPA / NetworkPolicy / CronJob**, optional NodePort & ServiceMonitor example). Entry points: **`k8s/scripts/demo-hub.sh`**, **`k8s/scripts/stop-start-all-k8s.sh`**, **`k8s/scripts/port-forward-demo-hub.sh`**. |
